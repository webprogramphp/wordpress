...
if ( ! class_exists( 'Nexas_About' ) ) {
	...
	class Nexas_About {
		...
		public static function init( $config ) {
			if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Nexas_About ) ) {
				self::$instance = new Nexas_About;
		...
		public function configure() {
			...
			$this->menu_name     = isset( $this->config['menu_name'] ) ? $this->config['menu_name'] : sprintf( esc_html__( 'About %s', 'nexas' ), $this->theme_name );
		...
		public function hooks() {
			...
			// Dismiss AJAX.
			add_action( 'wp_ajax_nexas_dismiss', array( $this, 'dismiss_callback' ) );
			add_action( 'wp_ajax_nopriv_nexas_dismiss', array( $this, 'dismiss_callback' ) );
		}
		...
		public function render_page() {
			?>
			<div class="wrap about-wrap nexas-about-wrap">
				...
				<?php endif; ?>
				<a href="<?php echo esc_url('https://paragonthemes.com','nexas') ?>" target="_blank"><div class="wp-badge"></div></a>
		...
		public function render_current_tab_content() {
			...
			if ( method_exists( $this, $method ) ) {
				...
				printf( esc_html__( '%s() method does not exist.', 'nexas' ), $method );
			}
		}
		...
		public function getting_started() {
			...
			?>
			<div class="feature-section nexas-section nexas-section-getting-started three-col">
				<?php foreach ( $content as $item ) : ?>
					<?php $this->render_grid_item( $item ); ?>
				<?php endforeach; ?>
			</div><!-- .feature-section .nexas-section -->
			<?php
		}
		...
		public function support() {
			...
			?>
			<div class="feature-section nexas-section nexas-section-support three-col">
				<?php foreach ( $content as $item ) : ?>
					<?php $this->render_grid_item( $item ); ?>
				<?php endforeach; ?>
			</div><!-- .feature-section .nexas-section -->
		...
		public function useful_plugins() {
			...
			?>
			<div class="nexas-section nexas-section-useful-plugins">
				...
				<?php if ( true !== $tgmpa_complete ) : ?>
					<a href="<?php echo esc_url( $tgmpa_url ); ?>" class="button button-primary"><?php esc_html_e( 'Manage Plugins', 'nexas' ); ?></a>
				<?php endif; ?>
		...
		public function demo_content() {
			...
			?>
			<div class="nexas-section nexas-section-demo-content">
		...
		public function assets( $hook ) {
			...
			if ( in_array( $hook, array( 'themes.php', 'appearance_page_' . $this->page_slug ), true ) ) {
				wp_enqueue_style( 'nexas-about', get_template_directory_uri() . '/assets/css/about.css', array(), '1.0.0' );
				wp_enqueue_script( 'nexas-about', get_template_directory_uri() . '/assets/js/about.js', array(), '1.0.0' );
			}
		...
		public function display_admin_notice() {
			...
			$dismiss_status = get_user_meta( $user_id, 'nexas_dismiss_status', true );
			?>
			<?php if ( current_user_can( 'edit_theme_options' ) && 'themes' === $screen_id && 1 !== absint( $dismiss_status ) ) : ?>
				<div class="nexas-about-notice notice notice-about">
					<?php echo $this->notice; ?>
				</div><!-- .nexas-about-notice -->
			<?php endif; ?>
			<?php
		}
		...
		public function dismiss_callback() {
			...
			if ( false === wp_verify_nonce( $wpnonce, 'nexas_dismiss_nonce' ) ) {
				wp_send_json( $output );
			}

			update_user_meta( $userid, 'nexas_dismiss_status', 1 );
